...
for n_clusters in range_n_clusters:
    ...
    print("NMI score: %.6f" % normalized_mutual_info_score(y, cluster_labels, average_method='geometric'))
...
for n_clusters in range_n_clusters:
    ...
    print("For n_clusters =", n_clusters,
          "The average silhouette_score is :", silhouette_avg)
    print("The NMI score is: %.6f" % normalized_mutual_info_score(y, cluster_labels, average_method='geometric'))
...
def plot_learning_curve(estimator, title, X, y, ylim=None, cv=None,
    ...
    train_sizes, train_scores, test_scores = learning_curve(
        estimator, X, y, cv=3, n_jobs=n_jobs, train_sizes=train_sizes)
...
for n_clusters in range_n_clusters:
    ...
    #colors = cm.spectral(cluster_labels.astype(float) / n_clusters)
    colors = plt.get_cmap('Spectral')(cluster_labels.astype(float) / n_clusters)
    ax2.scatter( X_norm.iloc[:, 3], X_norm.iloc[:, 1], marker='.', s=30, lw=0, alpha=0.7,
                c=colors, edgecolor='k')
    ...
    # Draw white circles at cluster centers
    ax2.scatter(centers[:, 3], centers[:, 1], marker='o',
                c="white", alpha=1, s=200, edgecolor='k')

    for i, c in enumerate(centers):
        ax2.scatter( c[3], c[1], marker='$%d$' % i, alpha=1,
                    s=50, edgecolor='k')
...
for n_clusters in range_n_clusters:
    ...
    #colors = cm.spectral(cluster_labels.astype(float) / n_clusters)
    colors = plt.get_cmap('Spectral')(cluster_labels.astype(float) / n_clusters)
    plt.scatter( X_norm.iloc[:, 3], X_norm.iloc[:, 1], marker='.', s=30, lw=0, alpha=0.7,
                c=colors, edgecolor='k')
    ...
    # Draw white circles at cluster centers
    plt.scatter(centers[:, 3], centers[:, 1], marker='o',
                c="white", alpha=1, s=200, edgecolor='k')

    for i, c in enumerate(centers):
        ax.scatter( c[3], c[1], marker='$%d$' % i, alpha=1,
                    s=50, edgecolor='k')
...
for color, i in zip(colors, [3,6]):
    plt.scatter(X_r[y == i, 0], X_r[y == i, 1], color=color, alpha=.8, lw=lw, label=i)
plt.legend(loc='best', shadow=False, scatterpoints=1)
plt.title('PCA of Yeast dataset')
plt.show()
plt.close()
...
for color, i in zip(colors, [3,6]):
    plt.scatter(X_r[y == i, 0], X_r[y == i, 1], color=color, alpha=.8, lw=lw, label=i)
plt.legend(loc='best', shadow=False, scatterpoints=1)
plt.title('ICA of Yeast dataset')
plt.show()
plt.close()
...
rca = GaussianRandomProjection(n_components=6, random_state=10)
X_r = rca.fit_transform(X)
X_rca = X_r
...
for color, i in zip(colors, [3,6]):
    plt.scatter(X_r[y == i, 0], X_r[y == i, 1], color=color, alpha=.8, lw=lw, label=i)
plt.legend(loc='best', shadow=False, scatterpoints=1)
plt.title('Random Projection of Yeast dataset')
plt.show()
plt.close()
...
plt.legend(loc='best', shadow=False, scatterpoints=1)
plt.title('Chi square feature selection of Yeast dataset')
plt.show()
plt.close()
